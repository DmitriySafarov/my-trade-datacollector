"""DB-backed integration tests for BinanceDepthCollector."""
...
from src.collectors.binance.depth_collector import BinanceDepthCollector
from src.collectors.binance.depth_parsing import SOURCE_ID

FIXTURE_PATH = (
    Path(__file__).resolve().parents[2]
    / "fixtures"
    / "binance"
    / "ws_depth_messages.json"
)
...
# ---------------------------------------------------------------------------
# WS mock helpers (same pattern as test_mark_price_collector.py)
# ---------------------------------------------------------------------------
...
async def _wait_for_rows(pool: Any, count: int, timeout: float = 5.0) -> None:
    ...
    while asyncio.get_event_loop().time() < deadline:
        async with pool.acquire() as conn:
            n = await conn.fetchval("SELECT count(*) FROM bn_depth")
        if n >= count:
            return
...
@pytest.mark.asyncio
async def test_collector_writes_depth_on_stop(
    migrated_db: dict[str, object],
) -> None:
    """Messages received before stop() are flushed to bn_depth."""
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT time, source, symbol, first_update_id, final_update_id,
                   last_update_id, bids, asks, snapshot_hash,
                   best_bid_price, best_ask_price, payload, ingested_at
            FROM bn_depth
            ORDER BY symbol
            """
        )
    ...
    assert [r["last_update_id"] for r in rows] == [987654321, 123456789]
    assert [r["best_bid_price"] for r in rows] == [84200.10, 3020.50]
    assert [r["best_ask_price"] for r in rows] == [84200.20, 3020.60]
    for row in rows:
        assert row["time"].tzinfo is not None
        assert row["ingested_at"].tzinfo is not None
        assert row["first_update_id"] is None
        assert row["final_update_id"] is None
        ...
        assert payload["lastUpdateId"] == row["last_update_id"]


@pytest.mark.asyncio
async def test_collector_deduplicates_replayed_depth(
    migrated_db: dict[str, object],
) -> None:
    """Replay guard trigger rejects duplicate snapshots for same source+symbol."""
    pool = migrated_db["pool"]
...
async def test_collector_health_snapshot(
    ...
    assert snap["name"] == "binance_depth"
    assert snap["source_ids"] == [SOURCE_ID]
    assert "writer" in snap
    assert snap["writer"]["name"] == "binance_depth_writer"
...
async def test_collector_invalid_payload_quarantined(
    ...
    invalid = {
        ...
        "data": {"lastUpdateId": 100},  # missing E, bids, asks
    }
    ...
    async with pool.acquire() as conn:
        count = await conn.fetchval("SELECT count(*) FROM bn_depth")
    assert count == 1
