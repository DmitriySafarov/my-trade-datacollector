...
@pytest.mark.asyncio
async def test_silver_market_views_drop_missing_key_legacy_ws_rows_after_repair(
    tmp_path: Path,
) -> None:
    async with upgrade_db(tmp_path) as pool:
        async with pool.acquire() as connection:
            await connection.execute(
                ...
                VALUES
                    ($1, 'hl_ws_asset_ctx', 'ETH', 0.01, 1500.0, 2000.0, NULL, $4),
                    ($2, 'hl_ws_asset_ctx', 'ETH', 0.01, 1500.0, 2000.0, NULL, $5),
                    ($3, 'hl_ws_asset_ctx', 'ETH', 0.02, 1600.0, 2001.0, 'ctx-1', $6)
                """,
                utc("2026-03-22T03:00:00+00:00"),
                utc("2026-03-22T03:00:00.001+00:00"),
                utc("2026-03-22T03:05:00+00:00"),
                utc("2026-03-22T03:05:00.001+00:00"),
            ...
            await connection.execute(
                ...
                VALUES
                    ($1, 'hl_ws_l2book', 'ETH', NULL, 99.0, 5.0, 100.0, 3.0, 1.0, $4),
                    ($2, 'hl_ws_l2book', 'ETH', NULL, 99.0, 5.0, 100.0, 3.0, 1.0, $5),
                    ($3, 'hl_ws_l2book', 'ETH', 'snap-1', 101.0, 4.0, 102.0, 2.0, 1.0, $6)
                """,
                utc("2026-03-22T04:00:00+00:00"),
                utc("2026-03-22T04:00:00.001+00:00"),
            ...
            await connection.execute(
                ...
                VALUES
                    ($1, 'bn_ws_depth', 'ETHUSDT', NULL, '[["200","2"]]'::jsonb, '[["201","1"]]'::jsonb, 200.0, 201.0, $4),
                    ($2, 'bn_ws_depth', 'ETHUSDT', NULL, '[["200","2"]]'::jsonb, '[["201","1"]]'::jsonb, 200.0, 201.0, $5),
                    ($3, 'bn_ws_depth', 'ETHUSDT', 101, '[["202","2"]]'::jsonb, '[["203","1"]]'::jsonb, 202.0, 203.0, $6)
                """,
                utc("2026-03-22T05:00:00+00:00"),
                utc("2026-03-22T05:00:00.001+00:00"),
                utc("2026-03-22T05:05:00+00:00"),
                utc("2026-03-22T05:10:00+00:00"),
                utc("2026-03-22T05:11:00+00:00"),
                utc("2026-03-22T05:12:00+00:00"),
            )

        await compress_tables(pool, "hl_asset_ctx", "hl_l2book", "bn_depth")
        ...
        async with pool.acquire() as connection:
            ...
            orderbook_rows = await connection.fetch(
                """
                SELECT time, source, exchange, coin, best_bid, best_ask
                FROM v_orderbook
    ...
    assert [dict(row) for row in funding_rows] == [
        {
            "time": utc("2026-03-22T03:05:00+00:00"),
    ...
    assert [dict(row) for row in orderbook_rows] == [
        {
            "time": utc("2026-03-22T05:05:00+00:00"),
            "source": "bn_ws_depth",
            "exchange": "binance",
            "coin": "ETH",
            "best_bid": 202.0,
        ...
        {
            "time": utc("2026-03-22T04:05:00+00:00"),
            "source": "hl_ws_l2book",
            "exchange": "hyperliquid",
            "coin": "ETH",
            "best_bid": 101.0,
