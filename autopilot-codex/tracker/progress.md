...
- Completed `P3.3` by implementing the Binance Futures markPrice @1s WebSocket collector for ETH + BTC, writing to `bn_mark_price` Bronze hypertable with SHA-256 event_hash replay dedup.
- Components: `mark_price_parsing.py` (120 lines) with strict validation, SHA-256 event_hash, premium computation (`mark_price - index_price`), optional `estimated_settle_price`, `next_funding_time` zero → `None`; `mark_price_storage.py` (36 lines) with COPY-based storage; `mark_price_collector.py` (110 lines) subclassing new `BinanceWsCollectorBase`; per-symbol `<symbol>@markPrice@1s` stream handlers; runtime registration in `collectors/__init__.py`.
- Extracted shared infrastructure to reduce DRY violations: `binance_ws_collector_base.py` (122 lines) with identical lifecycle logic (start, stop, wait_ready, health_snapshot, _stop_manager, _extract_error) shared by both Binance WS collectors; `binance_field_helpers.py` (130 lines) with shared field extraction helpers (require_str, require_bool, require_float, require_positive_float, require_nonneg_float, optional_nonneg_float, require_int, require_positive_int, require_nonneg_int, ms_to_datetime) with configurable `context` for error messages. Refactored `agg_trade_collector.py` and `agg_trade_parsing.py` to use shared modules. Removed dead `_run_task` field from `agg_trade_collector.py`.
- No new migrations needed — `bn_mark_price` Bronze table (migration 0004), replay guard trigger (0013/0019) on event_hash, unique index, and compression policy all pre-existed.
- Added 41 unit tests for parsing (happy path, symbol validation, missing fields, numeric validation, timestamp validation, payload type) and 5 DB-backed integration tests (flush-on-stop with full column verification, replay dedup via event_hash, timer-triggered flush, health snapshot, invalid payload quarantine). Updated runtime factory test to expect 5 collectors.
- Decisions: COPY-based storage (high-throughput @1s WS data); SHA-256 event_hash for replay dedup; `estimated_settle_price` optional (absent/empty → None); mark_price/index_price accept zero; funding_rate allows negative; `next_funding_time` zero → None.
- Validation: `./.venv/bin/ruff check .`, `./.venv/bin/ruff format --check .`, and `./.venv/bin/python -m pytest tests/ -v` all passed with `473 passed in 189s`.
- Next step: start `P3.4` Binance depth20 @100ms collector — ETH + BTC (source: bn_ws_depth).

- Completed `P3.4` by implementing the Binance Futures depth20 @100ms WebSocket collector for ETH + BTC, writing partial book depth snapshots to `bn_depth` Bronze hypertable with SHA-256 snapshot_hash replay dedup.
- Components: `depth_parsing.py` (150 lines) with strict [price, qty] level validation, SHA-256 snapshot_hash, derived best_bid_price/best_ask_price, symbol passed externally (partial depth payloads lack `s` field); `depth_storage.py` (36 lines) with COPY-based storage; `depth_collector.py` (105 lines) subclassing `BinanceWsCollectorBase` with per-symbol `<symbol>@depth20@100ms` stream handlers; runtime registration in `collectors/__init__.py`.
- No new migrations needed — `bn_depth` Bronze table (migration 0004), replay guard trigger (0013/0019), unique indexes on `(source, time, symbol, last_update_id)` and `(source, time, symbol, snapshot_hash)`, and compression policy all pre-existed.
- Added 43 unit tests for parsing (happy path, symbol validation, missing fields, level validation, timestamp validation, payload type) and 5 DB-backed integration tests (flush-on-stop with full column verification, replay dedup via replay guard, timer-triggered flush, health snapshot, invalid payload quarantine). Updated runtime factory test to expect 6 collectors.
- Decisions: COPY-based storage (high-throughput @100ms WS data); `first_update_id`/`final_update_id` set to None (only used by diff depth streams); empty bids/asks arrays accepted (valid edge case, best prices → None); replay guard uses `COALESCE(final_update_id, last_update_id, snapshot_hash)` with `last_update_id` as primary dedup key.
- Validation: `./.venv/bin/ruff check .`, `./.venv/bin/ruff format --check .`, and `./.venv/bin/python -m pytest tests/ -v` all passed with `521 passed in 190s`.
- Next step: start `P3.5` Binance forceOrder (liquidations) collector — ETH + BTC (source: bn_ws_liquidations).
